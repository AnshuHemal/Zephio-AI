...
async function runGenerationWorker({
  ...
  for (const page of pages) {
    ...
    // ── Per-page error recovery: if one page fails, keep going ──────────
    try {
...
- Context from previous pages : ${previousPagesContext}
    ...
    emit(
      ...
      // One page failed — log it, emit a partial error, and continue with remaining pages
      if (pageError instanceof AbortError) throw pageError; // re-throw abort
      console.error(`[generation] Page "${page.name}" failed:`, pageError?.message);
      emit(
        writer,
        "generation",
        {
          status: "generating",
          currentPageId: page.id,
          pageError: page.id, // signal this page failed
          pages: pages.map((p: any) => ({
            id: p.id,
            name: p.name,
            done: generationPages.some((gp: any) => gp.name === p.name),
          })),
        },
        { id: "gen-card" },
      );
  ...
  const summaryResult = await insforge.ai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are Zephio, an AI web design agent. You just finished building pages.
      ...
      selectedPage && isRegen
        ...
        ${stylePreset ? `${stylePreset}\n\n` : ""}USER REQUEST: "${latestUserMessage}"OUTPUT RAW JSON ONLY.`.trim(),
            ...
            if (isRegen && selectedPageId) {
              ...
              await incrementCredits(insforge, user.id, credits).catch(() => {});
              return;
            }
            ...
            // Emit updated credits to client so badge refreshes
            const updatedCredits = await getOrCreateCredits(insforge, user.id).catch(() => null);
            if (updatedCredits) {
              emit(writer, "credits-updated", {
                used: updatedCredits.used,
                remaining: Math.max(0, 10 - updatedCredits.used),
                limit: 10,
